...
import { genFilteredTopPostsData } from "../server/utils/post_fetch.server.js";
import {
  GeneralData,
  PostData,
  ScoreBoardEntry,
...
import { genPostsForSubreddit } from "../server/controllers/posts.server.js";
...
Devvit.addCustomPostType({
  ...
  render: (context) => {
    ...
    const {
      ...
      const subData = await genFilteredTopPostsData(context, subreddit.name);
      return {
        userId: user?.id ?? null,
    ...
    if (step === -1) {
      return <Intro step={step} setStep={setStep} />;
    }
    ...
    const score_component =
      score_loading === true ? (
        <text alignment="start top" style="heading">
          Account Balance: Calulating...
        </text>
      ) : (
        <text alignment="start top" style="heading">
          Account Balance: {current_score}
        </text>
      );
    ...
    const last_score_component =
      last_score_loading === true ? (
        ...
        <text
          ...
          style="heading"
          // grow={true}
          color={(last_score_update ?? 0) > 0 ? "green" : "red"}
        >
          P/L: {last_score_update}
        </text>
      );
    ...
    const post1 = step !== -1 && subData.posts[step * 2];
    const post2 = step !== -1 && subData.posts[step * 2 + 1];
    ...
    const handleCashout = (): void => {
      ...
      if (cashedOut !== true && generalDataPayload.username && current_score) {
              ...
              {ZoomPost != null ? (
                ...
                <>
                  ...
                  >
                    ...
                    >
                      {score_component}
                      {postBuy === true && last_score_component}
                    </vstack>
                    <hstack
                      alignment="center middle"
                      width="60%"
